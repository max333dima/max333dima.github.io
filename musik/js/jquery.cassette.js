...
(function (window, $, undefined) {
  ...
  $.Cassette.defaults = {
    ...
    songs: ['music-1', 'music-2', 'music-3', 'music-4'],
  ...
  $.Cassette.prototype = {
    ...
    _createPlayer: function () {
      ...
      this.$audioEl = $(
        '<audio id="audioElem"><span>' + this.options.fallbackMessage + '</span></audio>',
      );
    ...
    _createControls: function () {
      ...
      this.$cPlay = $('<li class="vc-control-play">Wiedergabe<span></span></li>');
      this.$cRewind = $('<li class="vc-control-rewind">Zurück<span></span></li>');
      this.$cForward = $('<li class="vc-control-fforward">Vor<span></span></li>');
      this.$cStop = $('<li class="vc-control-stop">Stopp<span></span></li>');
      this.$cSwitch = $('<li class="vc-control-switch" id="wechseln">Wechseln<span></span></li>');
    ...
    _switchSides: function () {
      if (this.isMoving) {
        alert('Bitte den Player stoppen, bevor die Seite gewechselt wird.');
        return false;
      }
      ...
      // Toggle a class instead of inline transform so the responsive scale
      // (CSS `transform: ... scale(var(--scale))`) is preserved during the flip.
      if (this.currentSide === 1) {
        this.currentSide = 2;

        this.$theTape.addClass('vc-tape-flipped');

        setTimeout(function () {
          _self.$tapeSideA.hide();
          _self.$tapeSideB.show();

          _self._resetSideToStart();
        }, 200);
      } else {
        this.currentSide = 1;

        this.$theTape.removeClass('vc-tape-flipped');
    ...
    // Position the new side at the very beginning so pressing Play starts the
    // first track of that side (without this, the side keeps the previous side's
    // wheel state and reads as "at the end", causing Play to no-op).
    _resetSideToStart: function () {
      this.cntTime = 0;
      this.timeIterator = 0;
      this._resetElapsed();
      this._setSidesPosStatus('start');
      var wheelVal = this._getWheelValues(0);
